...
const express = require('express'), app = express(), port = process.env.PORT || 8888;
...
app.get('/jpgs', function(req, res) {
	var content=JSON.parse(fs.readFileSync("jpegs.json" , 'utf8'));
	if(req.query.pretty!=undefined) {
		res.setHeader('Content-Type', 'plain/text');
		res.setHeader('Content-Disposition','inline');
		res.send(JSON.stringify(content,null,3));
...
console.log('(NodeJS Todolist - Movies) RESTful API server started on: ' + port);
